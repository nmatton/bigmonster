...
class BigMonster extends Table
{
   ...
   */
    ...
    {
        echo floatval(self::setStat(67015225, 'explorer', 2356485));
    }
    ...
    protected function moveCardFromHandToBoard($whichMove, $player_id)
    {
        ...
        $res = array(
            'card_id' =>  $this->dbcard[$id]['card_id'],
    ...
    function selectShip($ship_player_id, $rem_cards_str, $sel_card)
    {
        ...
        if ($ship_player_id == 0) {
            ...
            self::NotifyAllPlayers(
                "cardsOnShip",
                clienttranslate('${player_name} put the rest of his cards to ${player_ship_name} ship'),
                array(
    ...
    function argplaceTile()
    {
        ...
        foreach ($player_id_list as $player_id) {
            ...
            if (!empty($played_tile)) {
                ...
                $possible_moves['_private'][$player_id] = array(
    ...
    */
    ...
    function st_newRound()
    {
        ...
        if ($currentRound == 0) {
            ...
            foreach ($explorers as $player_id => $explorer_id) {
                self::NotifyAllPlayers(
                    "selectedExplorers",
                    clienttranslate('${player_name} selected ${explorer_name}'),
                    array(
                        "player_name" => self::getPlayerNameById($player_id),
    ...
    function st_endTurn()
    {
        ...
        foreach ($last_played_cards as $card_id => $value) {
            ...
            self::NotifyAllPlayers(
                "playedTiles",
                clienttranslate('${player_name} played an ${monster_type_name} monster'),
                array(
                    "player_name" => self::getPlayerNameById($player_id),
            ...
            if (intval($medal_details['player_id']) === 0 and $medal_details['type'] != 'lowest') {
                ...
                if (strlen($sucess_player_id > 0)) {
                    ...
                    foreach ($list_players as $player_id) {
                        ...
                        self::NotifyAllPlayers(
                            "wonMedal",
                            clienttranslate('${player_name} won the "${medal_name}" medal!'),
                            array(
                                "player_name" => self::getPlayerNameById($player_id),
            ...
            switch ($statename) {
                ...
                case 'tileSelection':
                    ...
                    if (count($cards) > 1) {
                        ...
                        self::NotifyAllPlayers(
                            "cardsOnShip",
                            clienttranslate('${player_name} put the last card to discard'),
                            array(
                                "player_name" => self::getPlayerNameById($active_player),
                                "player_id" => $active_player,
                                "player_ship_id" => 0,
